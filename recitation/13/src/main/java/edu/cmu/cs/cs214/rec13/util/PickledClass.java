...
public class PickledClass implements Serializable {
    ...
    public PickledClass(Class<?> classObj) {
        ...
        InputStream is = classObj.getResourceAsStream(name.substring(name.lastIndexOf('.') + 1) + ".class");
        try {
    ...
    public boolean equals(Object o) {
        ...
        PickledClass that = (PickledClass) o;

        return name.equals(that.name) && Arrays.equals(classFile, that.classFile);
    }
